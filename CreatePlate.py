...
cur.execute('CREATE TABLE IF NOT EXISTS plate (position INTEGER PRIMARY KEY, row CHAR(1), col CHAR(2))')
...
for row in rows:
    for col in cols:
        values = str(count) + ', ' + '\'' + str(row) + '\'' + ' ,' + '\'' + str(col) + '\''
        cur.execute('INSERT INTO plate (position, row, col) VALUES (' + values + ');') 
        count += 1
conn.commit()
